...
class OptitrackPoseCovPublisher : public rclcpp::Node
{
public:
  OptitrackPoseCovPublisher()
  : Node("optitrack_posecov_publisher")
  {
    // Subscribe to the OptiTrack rigid bodies topic
    subscription_ = this->create_subscription<mocap4r2_msgs::msg::RigidBodies>(
      "/rigid_bodies", 10,
      std::bind(&OptitrackPoseCovPublisher::rigid_bodies_callback, this, std::placeholders::_1));

    RCLCPP_INFO(this->get_logger(), "OptiTrack PoseCov Publisher started");
  }
...
private:
  void rigid_bodies_callback(const mocap4r2_msgs::msg::RigidBodies::SharedPtr msg)
  {
    ...
    for (const auto& rigid_body : msg->rigidbodies)
    {
      ...
      // Create topic name as turtleX/pose_with_cov where X is the rigid body name
      std::string topic_name = "turtle" + body_name + "/pose_with_cov";
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<OptitrackPoseCovPublisher>());
  rclcpp::shutdown();
  return 0;
}
